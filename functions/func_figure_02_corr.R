...
func_figure_02_corr <- function(data){
  ...
  #搞好了mat1  
  if (datagroup == 1) {
    xp = mat1[211:294,211:294]
  }else if (datagroup == 2) {
    xp = mat1[22:105,22:105]
  }
  fk = 21
  fi = 4
  xrev = apply(xp, 2, rev)
  ...
  df1$x = rep(rep(aglist,each = 4*fk),4)
  df1$y = rep(rep(aglistinv,4),fk*4)

  if (datagroup == 1) {
    df1$pop_r = rep(rep(coulist[11:14],each = fk),fk*4)
    df1$pop_e = rep(coulist[11:14],each = fk^2*4)
    }else if (datagroup == 2) {
      df1$pop_r = rep(rep(coulist[2:5],each = fk),fk*4)
      df1$pop_e = rep(coulist[2:5],each = fk^2*4)
      }
  #longData<-longData[longData$value!=0,]
  
  g1=ggplot(df1, aes(x = x, y = y)) + 
    geom_tile(aes(fill=value)) + facet_grid(cols = vars(pop_e),rows = vars(pop_r))+
    scale_fill_gradient2(low = "blue2", mid = "white", high = "red2", midpoint = 0)+
    labs(x="Age Group", y="Age Group", title="SVAR") +
