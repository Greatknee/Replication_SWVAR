func_figure_04_draw <- function(fit){
  f0 = fit$f0
  fdm = fit$fdm
  VAR_1 = fit$VAR_1
  f4 = fit$f4
  ...
  p = pfore1 + geom_line(aes(color = Model))+xlim(1,tstep)+
    xlab(expression(atop("Year"~italic(h)~'from jump-off year')))+ylab(expression(RMSFE))+facet_wrap(~ Pop,ncol = 3)+
