...
func_table_01_insample <- function(data){
  ...
  VAR_1 = fitstar_rev(datar = datar,'stack')
  VAR_1$summary
  # x <- 1:(glok*gloi)
  # y <- (gloi*glok):1
  ...
  VAR_2 = fitstar_rev(datar = datar,'sep')
  VAR_2$summary
...
func_table_01_insample_m <- function(data){
  set.seed(123)
  #datapre
  datagroup = data$group
  coulist = data$coulist
  ...
  diffpredy = aperm(apply(datar, c(1, 3), diff),c(2,1,3)) - aperm(apply(predy, c(1, 3), diff),c(2,1,3))
  for (i in 1:gloi) {
    sig = sigseqr[,((1:glok)+glok*(i-1))]
    templ = sapply(as.data.frame(dfy[,,i]),wws,S=sig)
    l = l - 1/2*sum(templ)
  }
  ...
  BIC
  cu1 = list()
  cu1$RMSE = sqrt(mean(dfy^2))
  cu1$RMSE_rate =  sqrt(mean(diffpredy^2))
  cu1$edf = orin
  cu1$l = l
  cu1$AIC = AIC
  cu1$BIC = BIC
  ...
  VAR_1 = fitstar(datar = datar,'stack')
  VAR_1$summary
  #
  VAR_2 = fitstar(datar = datar,'sep')
  VAR_2$summary
  ...
  #customsummary(VAR_1,K = gloKc,T = gloTc,s = sig_c)
  #cu3 = customsummary2(VAR_3,K = glok,T = glot,gloi,s = sigseq)
  cu3p = customsummary_BICinrate(VAR_3,K = glok,T = glot,I = gloi,rawdata = datamat_c,s = sigseqr)
  cu3p$sl
  cu4p = customsummary_BICinrate(VAR_4,K = glok,T = glot,I = gloi,rawdata = datamat_c,s = sigseqr)
  cu4p$BIC
  ...
  # weight age only need to multiply before the X
  #datapre
  tempfilepath = paste('param/insample',datagroup,'.csv',sep = '')
  ...
  if (is.null(weight)) {
    ...
    besta
    bests
    bestagroup = aseq[besta]
    bestsgroup = sseq[bests]
    write.csv(t(rbind(bestagroup, bestsgroup)),file =tempfilepath)
  ...
  for (i in 1:gloi) {
    ...
    for (i3 in 1:glok) {
      for (j3 in 1:glok) {
        wa[i3,j3] = exp(abs(i3-j3)/bestagroup[i])
      }
    }
    
    scaledws = c(min(tempws),bestsgroup[i]*tempws)/sum(tempws)
    result_list = list()
    for (k in 1:gloi) {
      result_list[[k]] <-  scaledws[k] *wa
    }
  ...
  for (i in 1:gloi) {
    s = sigseqr[,((1:glok)+(i-1)*glok)]
    tempdfy = looppred[,((1:glok)+(i-1)*glok)] + t(datar[,1:(glot-1),i]) -  t(datar[,2:glot,i])
    templ = sapply(as.data.frame(t(tempdfy[2:(glot-1),])),wws,S=s)
    l4[i] =  - 1/2*sum(templ)
  }
