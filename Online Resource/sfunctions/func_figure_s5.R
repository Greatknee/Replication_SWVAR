...
func_figure_s51 <- function(data){
  ...
  index = 1:8
  tempi = length(index)
  LE = as.vector(e0[,index]) 
  year = rep(seq(2000,2049,1),tempi)
  pop= rep(coulist[index],each = 50)
  df1 = data.frame(LE)
  df1$upper = as.vector(emax[,index])
  df1$lower = as.vector(emin[,index])
  df1$emid = as.vector(emid[,index])
  df1$year = year
  df1$Pop = pop
  df1$model = rep('SWVAR',tempi*50)
  
  df1$ell = as.vector(emidll[,index])
  df1$lowerll = as.vector(eminll[,index])
  df1$upperll = as.vector(emaxll[,index])
  year0 = rep(seq(ystr,yete,1),tempi)
  df2 = data.frame(year0)
  df2$eold = as.vector(e0_true[,index])
  df2$Pop = rep(coulist[index],each = yete+1-ystr)
  
  pfore2 <- ggplot()
  
  
  p1 = pfore2 +  geom_line(data = df2, aes(x=year0, y = eold,colour = 'Observed'),linewidth = 0.6)+  facet_wrap(~ df2$Pop)+
    ...
    ggtitle('Forecsting of Life Expectancy')+xlab('Year')+
  ...
  p2 = pfore2 +  geom_line(data = df2, aes(x=year0, y = eold,colour = 'Observed'),linewidth = 0.6)+  facet_wrap(~ df2$Pop)+
    geom_smooth(data = df1,method = 'loess',se= FALSE,aes(x = year, y = emid,colour = 'SWVAR'),linewidth = 1,span = 0.35,linetype = 1)+
    geom_smooth(data = df1,method = 'loess',se= FALSE,aes(x = year, y = lower),linewidth = 0.4,span = 0.35,linetype = 2,color='blue')+
    geom_smooth(data = df1,method = 'loess',se= FALSE,aes(x = year, y = upper),linewidth = 0.4,span = 0.35,linetype = 2,color='blue')+
    geom_smooth(data = df1,method = 'loess',se= FALSE,aes(x = year, y = ell,colour = 'Li-Lee'),linewidth = 1,span = 0.35,linetype = 1)+
