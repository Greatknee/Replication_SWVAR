...
func_figure_s11 <- function(data){
  ...
  df2$pop_r = rep(rep(coulist[tempcou],each = fk),fk*fi)
  df2$pop_e = rep(coulist[tempcou],each = fk^2*fi)
  ...
  df1$pop_r = rep(rep(coulist[1:7],each = fk),fk*fi2)
  df1$pop_e = rep(coulist[8:14],each = fk^2*fi1)
  ...
  aglist = seq(0,100,5)#as.character(seq(1,fk,1))
  aglistinv = seq(100,0,-5)#as.character(seq(fk,1,-1))
  df2$x = rep(rep(aglist,each = fk*fi1),fi2)
  df2$y = rep(rep(aglistinv,fi2),fk*fi1)
  df2$pop_r = rep(rep(coulist[1:7],each = fk),fk*fi2)
  df2$pop_e = rep(coulist[8:14],each = fk^2*fi1)
  ...
  return(list(g1 = g1, g2=g2, g3 = g3, g4 = g4,g5 = g5, g6=g6, g7 = g7, g8 = g8))
}
